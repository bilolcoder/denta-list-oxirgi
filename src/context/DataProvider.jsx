...
export const DataProvider = ({ children }) => {
        ...
        if (accessToken && savedPhone) {
            ...
            if (savedUser) {
                try {
                    ...
                    console.log('✅ User ma\'lumotlari yuklandi:', userObj);
                } catch (e) {
    ...
    // loginWithPhone funksiyasi - O'ZGARTIRILDI: navigate olib tashlandi
    const loginWithPhone = useCallback((phone, userObj = null) => {
        console.log('🔐 loginWithPhone chaqirildi:', { phone, userObj });
    ...
    const logout = useCallback(() => {
        ...
        // Navigatsiya emas, faqat state tozalanadi
    }, []);

    // updateUser funksiyasi
    const updateUser = useCallback((newUserData) => {
        setData(prev => {
            const updatedUser = prev.user ? { ...prev.user, ...newUserData } : newUserData;
    ...
    // Umumiy ma'lumotlarni saqlash
    useEffect(() => {
        const { user, ...dataWithoutUser } = data;
        localStorage.setItem('clinic_app_data', JSON.stringify(dataWithoutUser));
    }, [data]);
    ...
    const contextValue = {
        ...
        t,
        isAuthenticated,
        loginWithPhone, // BU FUNKSIYA MAVJUD
        logout,
        updateUser,
        getUser: () => data.user,
        getPhone: () => localStorage.getItem('userPhone'),
        setData: setData
    };
